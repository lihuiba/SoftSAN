...
static PyObject *
execute(PyObject *pyargc, PyObject *pyargv)//this function is used to execute
{
	int argc = PyInt_AsInt(pyargc);
	char *argv[];
...
//Define a method which can be used in python, {NULL, NULL} means end
static PyMethodDef pyds_functions[] = {
	{"scanparts", (PyCFunction)execute, PYDM_ARGS,
		"Rescans the partition talbe for the specified device.	"},
	{NULL, NULL}
};

//Initial a python module that it can be used in python
PyMODINIT_FUNC
initdm(void)
{
	PyObject *m;

	m = Py_InitModule("dm", pyds_functions);
}
